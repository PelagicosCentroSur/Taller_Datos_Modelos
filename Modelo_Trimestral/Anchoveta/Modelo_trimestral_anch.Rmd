...
- talla media de madurez (12 para sardina común confirmar!!)
...
- Mortalidad natural igual a 0.22 (trimestral, averiguar de donde se obtienen los parámetros de la relación logitud-peso) 
- Parámetros de crecimiento:

   - Loo = 20.8 (cm), (fijo)
   - k=0.11 (trimestral???), (fijo) 
   - Lr=2.36 (prior, es estimado por el modelo) (longitud de reclutas)
   - sr=1.0 (se usa para la matriz talla-talla, no recuerdo si tiene que ver con la amplitud, averiguar)
...
-  Selectividad logística para crucero se verano y otoño. 
...
tallas     <-as.numeric(strsplit(B[130],"\t")[[1]])
madurez    <-as.numeric(strsplit(B[496],"\t")[[1]])
pesos_med  <-as.numeric(strsplit(B[498],"\t")[[1]])
#-------------------------------------------------------------------------------
names_ind<-strsplit(B[11],"\t")
names_ind[[1]][1]<-"year"
rowind<-12:127 #filas de indices de abundancia
ncol_ind<-15
yrs<-length(rowind)
ind<-matrix(nrow=yrs,ncol=ncol_ind)
for(i in 1:yrs){ind[i,]<-as.numeric(strsplit(B[rowind[i]],"\t")[[1]])}
colnames(ind)<-names_ind[[1]][1:15]
#-------------------------------------------------------------------------------
#Composiciones de edad de la flota 
rowpfo<-c(134:249) #filas de proporcion de la flota observados
pf_obs<-matrix(nrow=yrs,ncol=length(tallas))
for(i in 1:length(rowpfo)){pf_obs[i,]<-as.numeric(strsplit(B[rowpfo[i]],"\t")[[1]])}
colnames(pf_obs)<-tallas
pf_obs<-pf_obs[,1:ntallas]
#Composiciones de edad de reclas
rowpR<-c(253:348,350:353,355:358,360:363,365:368,370:373) #filas de proporcion de reclas observados
pR_obs<-matrix(nrow=yrs,ncol=length(tallas))
for(i in 1:length(rowpR)){pR_obs[i,]<-as.numeric(strsplit(B[rowpR[i]],"\t")[[1]])}
colnames(pR_obs)<-tallas
pR_obs<-pR_obs[,1:ntallas]
#Composiciones de edad de pelaces
rowpP<-377:492 #filas de proporcion de pelaces observados
pP_obs<-matrix(nrow=yrs,ncol=length(tallas))
for(i in 1:length(rowpP)){pP_obs[i,]<-as.numeric(strsplit(B[rowpP[i]],"\t")[[1]])}
colnames(pP_obs)<-tallas
pP_obs<-pP_obs[,1:ntallas]
```
...
reclas_obs<-dataInd$B.rs %>% na_if(0)
pelaces_obs<-dataInd$B.pel %>% na_if(0)
...
tallamediaF<-as.numeric(strsplit(C[16]," ")[[1]])[2:117]
tallamediaR<-as.numeric(strsplit(C[19]," ")[[1]])[2:117]
tallamediaP<-as.numeric(strsplit(C[22]," ")[[1]])[2:117]
...
#Composiciones de edad de pelaces
rowselF<-168:283 #filas de proporcion de pelaces observados
...
selR<-as.numeric(strsplit(C[285]," ")[[1]])
selR<-data.frame(Tallas,sel=selR[2:38])

selP<-as.numeric(strsplit(C[287]," ")[[1]])
selP<-data.frame(Tallas,sel=selP[2:38])
