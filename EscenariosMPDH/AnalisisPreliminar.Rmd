...
```{r}
data1        <- lisread(here("codigo_admb_modificado","MAE323m.dat")) 
names(data1) <- str_trim(names(data1), side="right")
dat1         <- data1
rep1         <- reptoRlist(here("codigo_admb_modificado","MAE323m.rep"))
std1         <- read.table(here("codigo_admb_modificado","MAE323m.std"),header=T,sep="",na="NA",fill=T) 
```
