...
file.copy(c(here("codigo_admb_modificado_S","MAE323m.dat"),
            here("codigo_admb_modificado_S","MAE323m.tpl")),dir.esc)
...
mpdh_dens<-c(rep(0,12),
             245,19,12,66,0,3354,207,118,43,
             556,44,289,468,355,642,0,171,1655,397,36,0)

cvmpdh_dens<-c(rep(0.3,12),
               .182,.245,.333,.294,.30,.234,.245,.248,.543,
               .249,.267,.248,.245,.246,.230,.30,.246,.239,.229,.272,.30)

dtmpdh<-c(rep(0.16,12),
          .13,.15,.14,.14,.16,.16,.18,.18,.20,.23,.27,
          .24,.19,.24,.19,.16,.19,.14,.24,.27,.20) # dt variable

# 
# cvreclas<-c(rep(0.30,12),
#             .16,.16,.12,.15,.08,.14,.14,.20,.07,.11,.15,
#             .10,.06,.21,.05,.15,.13,.05,.07,.04,.04,NA,NA)

# cvpela<-c(rep(0.30,14),
#           .13,.3,.11,.17,.29,.12,.20,.12,.30,.10,.17,.07,.08,NA,NA)
# 
# cvmpdh<-c(rep(0.30,8),
...
dat1         <- data1
dat1$Ind[,7] <- 100 # cv mpdh
dat1$Ind[,10] <- dtmpdh # dt variable
dat1$nbloq_qmph <- 1 # bloques q
dat1$ano_inicio_nbloq_qmph <- 1991 # año inicio q
...
#------------------------------------------
#S9 CV pelaces=100, CV reclas=0.15, CV mph = 0.3, dt variable y q bloque
dat1         <- data1
dat1$Ind[,3] <- 0.15 # cv reclas
dat1$Ind[,5] <- 0.25 # cv pelaces
dat1$Ind[,7] <- 0.3 # cv mpdh
dat1$Ind[,10] <- dtmpdh # dt variable
dat1$nbloq_qmph <- 3 # bloques q
dat1$ano_inicio_nbloq_qmph <- c(1991,2010,2013) # año inicio q
...
#------------------------------------------
#S10 mpdj=densidad, CV = 0.3, dt variable y q bloque
dat1         <- data1
...
```{r}
data1        <- lisread(here("Sensibilidad_mpdh2023","MAE323m1.dat")) 
names(data1) <- str_trim(names(data1), side="right")
dat1         <- data1
rep1         <- reptoRlist(here("Sensibilidad_mpdh2023","MAE323m1.rep"))
std1         <- read.table(here("Sensibilidad_mpdh2023","MAE323m1.std"),header=T,sep="",na="NA",fill=T) 
```
...
caption1<-"\\label{tab1}Índices de abundancia con los coeficientes de variación utilizados en el modelo de evaluación de stock de sardina común."
...
```{r fig.height=2.5,fig.width=5,fig.cap="\\label{Fig2}Índices de biomasa observados (puntos) y estimados (línea) por el modelo de evaluación de stock de sardina común."}
...
dtMPDHvar <- data.frame(yearF,fechainicio,fechatermino,diascrucero,CVmpdh_var,yearBio,fraccionyear)
...
\begin{figure}[!htb]
\centering
\includegraphics[width=1\textwidth]{Figuras/distribucionEspacialHuevosSardina.png}
...
S9<-"S4 + CV C.verano=0.15 y CV C.otoño=0.25"
...
\newpage
```{r fig.cap="\\label{Fig34}Perfiles de verosimilitud de las fuentes de datos para el parámetro de reclutamiento medio ($R_0$), donde la línea horizontal representa el nivel crítico para el test $\\chi^2$.",fig.height=6,fig.width=7}
...
\newpage
```{r fig.height=6,fig.width=8, fig.cap="\\label{Fig35} Variables poblacionales de sardina común"}
BDplot<-BD[,1:3]%>%data.frame()%>%mutate(years=years,index="Reclutamientos") %>% melt(id=c("years","index"))
Rtplot<-Rt[,1:3]%>%data.frame()%>%mutate(years=years,index="Biomasa desovante") %>% melt(id=c("years","index"))
...
tittleleg<-"Escenarios"
#----------------------------------------------------------------------
p1<-ggplot(Ajustes2,aes(x=years,y=value/1000000,color=variable))+
  geom_line(aes(linetype=variable))+
  geom_point(aes(shape=variable),size=2.5)+xlim(2000,2024)+
  scale_color_manual(name=tittleleg,labels=labelsplot,values=colorplot)+
  scale_shape_manual(name=tittleleg,labels=labelsplot,values=shapeplot)+
...
```{r fig.cap="\\label{Fig34}Perfiles de verosimilitud de las fuentes de datos para el parámetro de reclutamiento medio ($R_0$), donde la línea horizontal representa el nivel crítico para el test $\\chi^2$.",fig.height=7,fig.width=7}
...
p2<-fig12.1(Rdata_perfils4,"Hito 2: S4",c(30000, 210000))
p3<-fig12.1(Rdata_perfils5,"Hito 2: S5",c(30000, 210000))
p4<-fig12.1(Rdata_perfils6,"Hito 2: S6",c(30000, 210000))
...
BDplot<-BD[,c(1,4:6)]%>%data.frame()%>%mutate(years=years,index="Reclutamientos") %>% melt(id=c("years","index"))
Rtplot<-Rt[,c(1,4:6)]%>%data.frame()%>%mutate(years=years,index="Biomasa desovante") %>% melt(id=c("years","index"))
...
# S3 cambios en CV reclas y pelaces
...
p1<-fig12.1(Rdata_perfils1,"Hito 2: S1",c(30000, 210000))
p2<-fig12.1(Rdata_perfils7,"Hito 2: S7",c(30000, 210000))
p3<-fig12.1(Rdata_perfils8,"Hito 2: S8",c(30000, 210000))
p4<-fig12.1(Rdata_perfils9,"Hito 2: S9",c(30000, 210000))
...
```{r fig.height=6,fig.width=8}
BDplot<-BD[,c(1,7:9)]%>%data.frame()%>%mutate(years=years,index="Reclutamientos") %>% melt(id=c("years","index"))
Rtplot<-Rt[,c(1,7:9)]%>%data.frame()%>%mutate(years=years,index="Biomasa desovante") %>% melt(id=c("years","index"))
...
#----------------------------------------------------------------------
labelsplot<-c("caso base","cv_reclas=100","cv_pelaces=100","cv_reclas=0.15, cv_pelaces=0.25, cv_mph=0.3 ")
colorplot<-c("black","red","green","blue")
linetypeplot<-c("solid",rep("dashed",3))
tittleleg<-"Escenarios"
#----------------------------------------------------------------------
...
tittleleg<-"Escenarios"
#----------------------------------------------------------------------
p1<-ggplot(Ajustes3,aes(x=years,y=value/1000000,color=variable))+
  geom_line(aes(linetype=variable))+
  geom_point(aes(shape=variable),size=2.5)+xlim(2000,2024)+
  scale_color_manual(name=tittleleg,labels=labelsplot,values=colorplot)+
  scale_shape_manual(name=tittleleg,labels=labelsplot,values=shapeplot)+
...
data3<-cbind(Datos[,c(1,2,4,6)],mpdh_dens*200)%>% melt(id="year")
data3[data3==0]<-NA

ggplot(data3,aes(x=year,y=log(value)))+xlim(2000,2024)+geom_point(aes(color=variable))+
  geom_smooth(aes(color=variable),alpha=.2)+
  ylab("Biomasas (millones de t)")+xlab("Años")+ggtitle("Sardina común centro-sur")+
  theme_bw()
