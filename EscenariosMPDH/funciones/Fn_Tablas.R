...
tb1<-function(label_tb,archivo.Rdata){
  ...
  dataInd[is.na(dataInd)] <- 0
	intcolumn <- c(TRUE, FALSE, FALSE, FALSE)  # This is hard to automate
  
  for (i in seq_len(ncol(dataInd))) {
    if (intcolumn[i])
      dataInd[[i]] <- format(dataInd[[i]])
    else
      dataInd[[i]] <- format(dataInd[[i]], big.mark = ".")
  }																   
  caption1<-"Estimaciones de biomasas  de los cruceros de Verano (RECLAS), 
  Otoño (PELACES) y crucero de huevos (MPDH) utilizadas en la evaluación de stock de "
...
captura descartada (toneladas) y captura total (toneladas) estimadas en año biológico de "
...
tb3<-function(label_tb,Rdata_H1,Rdata_H2){
  ...
  VarPobl1<- cbind(yearsbiol,
                   format(round(c(NA,VarPob_H1[VarPob_H1$indicador=='BD',]$value),0),nsmall=0, big.mark="."),
                   format(round(VarPob_H2[VarPob_H2$indicador=='BD',]$value,0),nsmall=0, big.mark="."),
                   format(round(c(NA,VarPob_H1[VarPob_H1$indicador=='BT',]$value),0),nsmall=0, big.mark="."),
                   format(round(VarPob_H2[VarPob_H2$indicador=='BT',]$value,0),nsmall=0, big.mark="."),
                   format(round(c(NA,VarPob_H1[VarPob_H1$indicador=='Rt',]$value),0),nsmall=0, big.mark="."),
                   format(round(VarPob_H2[VarPob_H2$indicador=='Rt',]$value,0),nsmall=0, big.mark="."),
                   formatC(round(c(NA,VarPob_H1[VarPob_H1$indicador=='Ft',]$value),3),decimal.mark = ",",digits = 3),
  ...
  biomasa total (t), reclutamientos (\\#) y  mortalidad por pesca ($año_{-1}$)  
...
tb3_H3<-function(label_tb,Rdata_H1,Rdata_H2,Rdata_H3){
  ...
  caption1<-"Estimaciones medias de las biomasa desovante (t), 
  biomasa total (t), reclutamientos (\\#) y  mortalidad por pesca ($año_{-1}$)  
  estimadas en las asesorías de septiembre 2022 (Hito 1), marzo 2023 (Hito 2) y julio (Hito 3). "
  # TABLA
  VarPobl1 %>%
    kbl(booktabs = T,format = "latex",position="h!",escape = F,align="c",
    ...
    kable_styling(latex_options = c("striped", "condensed"),
                  full_width = FALSE,font_size=8)%>%
    add_header_above(c('Año biológico'=1,
                       "Biomasa\ndesovante"=3,
                       "Biomasa total"=3,
                       "Reclutamientos"=3,
                       "Mortalidad\npor pesca"=3))
...
tb4<-function(label_tb,pbrHito1,pbrHito2,especie){
  ...
  caption1 <- "\\label{Tab21} Procedimiento de cálculo de los puntos biológicos de
...
tb4_H3<-function(label_tb,pbrHito1,pbrHito2,pbrHito3,especie){
  ...
  caption1 <- "\\label{Tab21} Procedimiento de cálculo de los puntos biológicos de
  referencia de biomasa (miles de t) estimados en las asesorías de septiembre 
  2022 (Hito 1), marzo 2023 (Hito 2) y julio 2023 (Hito 3), calculados siguiendo los pasos descritos 
  en la metodología de este informe. "
...
caption1<-"\\label{Tab21b}Puntos Biológicos de referencia (PBRs) y 
...
Estatus2<- data.frame(
  'Años'=yearsbiol,
  "F_FRMS_h1"=formatC(round(c(NA,IndStatus_H1[IndStatus_H1$indicador=='F_FRMS',]$value),3), decimal.mark = ","),
  "F_FRMS_h2"=formatC(round(c(IndStatus_H2[IndStatus_H2$indicador=='F_FRMS',]$value),3), decimal.mark = ","),
  "BD_BDRMS_h1"=formatC(round(c(NA,IndStatus_H1[IndStatus_H1$indicador=='BD_BDRMS',]$value),3), decimal.mark = ","),
  "BD_BDRMS_h2"=formatC(round(c(IndStatus_H2[IndStatus_H2$indicador=='BD_BDRMS',]$value),3), decimal.mark = ","),
  "Y_BT_h1"=formatC(round(c(NA,IndStatus_H1[IndStatus_H1$indicador=='Y_BT',]$value),3), decimal.mark = ","),
  "Y_BT_h2"=formatC(round(c(IndStatus_H2[IndStatus_H2$indicador=='Y_BT',]$value),3), decimal.mark = ","),
  "C_N_h1"=formatC(round(c(NA,IndStatus_H1[IndStatus_H1$indicador=='C_N',]$value),3), decimal.mark = ","),
  "C_N_h2"=formatC(round(c(IndStatus_H2[IndStatus_H2$indicador=='C_N',]$value),3), decimal.mark = ","))


caption1 <- "\\label{Tab22}Índices de reducción de F respecto de $F_{RMS}$ 
...
tb6_H3<-function(label_tb,Rdata_H1,Rdata_H2,Rdata_H3){
...
tb7<-function(label_tb,Rdata_proy_Hito,escRecl,especie,region){
  ...
  # TABLA 
  #-----------
  kbl(probEstatus, booktabs = T,format = "latex",position="t!",align="c",escape = FALSE,
      caption = paste(label_tb,caption1,especie,region,sep=" ")) %>% 
    kable_styling(latex_options = c("striped", "condensed"),
                  full_width = FALSE,font_size=10)%>% 
...
tb8_H2<-function(label_tb,Rdata_proy_Hito,escRecl,especie,region){
  ...
  caption1 <-"\\label{Tab26am}Proyección de la captura estimada para los años biológicos 
      2022/23 y 2023/24 considerando una mortalidad por pesca igual al $F_{RMS}$, 
      con sus respectivos escenarios de reclutamientos. El cálculo de la Captura 
      para el año calendario 2023 se obtiene como el promedio ponderado según la 
      estacionalidad semestral de la pesquería que a la fecha se asume 70\\% para el primer 
...
tb8b_H2<-function(label_tb,Rdata_proy_Hito,escRecl,especie,region){
  ...
  caption1 <-"\\label{Tab26bm}Captura año 2022/23 actualizada en Hito 2 (marzo 2023) y proyectada hacia
...
tb9_H2<-function(label_tb,Rdata_proy_Hito,escRecl,especie,region){
  ...
  caption1 <-"\\label{Tab26m2}Primera revisión de la CBA año calendario 2023 calculada bajo 
      $F_{RMS}$ con sus respectivos percentiles de captura entre 10\\% y 50\\% y 
      tres escenarios de reclutamientos."
  #-----------
  # TABLA 
...
tb10_sard_H2<-function(label_tb,Rdata_proy_Hito,escRecl,especie,region){
  ...
  caption1 <-"\\label{Tab27m}Primera revisión de la CBA año calendario 2023 considerando el
              descuento del 4\\% de descarte."
...
tb9B_H2<-function(label_tb,Rdata_proy_Hito,escRecl,especie,region){
  ...
  caption1 <-"\\label{Tab27bm}Primera revisión de la CBA año calendario 2023 calculada bajo 
      $F_{RMS}$ con sus respectivos percentiles de captura entre 10\\% y 50\\% y 
      tres escenarios de reclutamientos."
  #-----------
  # TABLA 
...
tb11B_H2<-function(label_tb,Rdata_proy_Hito,escRecl,especie,region){
  ...
  caption1 <-"\\label{Tab28bm}Nivel de resguardo de cada percentil de Captura al RMS."
...
tb12_sard_H2<-function(label_tb,Rdata_proy_Hito1,Rdata_proy_Hito2,escRecl,especie,region){
  ...
  tabladiferencias<--round(1-(CBAd_marzo/CBAd_sept),2)*100
...
tb13_sard_H2<-function(label_tb,Rdata_proy_Hito1,Rdata_proy_Hito2,escRecl,especie,region){
  ...
  tabladiferencias<--round(1-(CBAd_marzoD2SdR/CBAd_sept),2)*100
  colnames(tabladiferencias)<-percentiles
  row.names(tabladiferencias)<-escRecl
  
  
  
  caption1 <-"\\label{Tab27m_c2}Diferencias porcentuales entre 
...
tb14<-function(label_tb,comp,especie){
  ...
  caption1<-"\\label{t5_obj4} Comparación entre el caso base y escenarios alternativos de 
...
tb10_sard_C1<-function(label_tb,escRecl,especie,region){
  ...
  load(Rdata_proy_Hito2)
  CBAd_marzoS0 <- t(CBAd_marzo)
  load(Rdata_proy_Hito2S1)
  CBAd_marzoS1 <- t(CBAd_marzo)
  load(Rdata_proy_Hito2S2)
  CBAd_marzoS2 <- t(CBAd_marzo)
  load(Rdata_proy_Hito2S3)
  CBAd_marzoS3 <- t(CBAd_marzo)
  load(Rdata_proy_Hito2S4)
  CBAd_marzoS4 <- t(CBAd_marzo)
  
  
  tCBAd<-data.frame(escenarios,percentiles,
                    format(round(rbind(CBAd_marzoS1,CBAd_marzoS2,
                                CBAd_marzoS3,CBAd_marzoS4,CBAd_marzoS0),0),big.mark = '.'))
  ...
  caption1 <-"Estimación de CBA 2023 de los cuatro escenarios de supuesto de captura 2022/23
              considerando el 
              descuento del 4\\% de descarte. Criterio 1."
...
tb10_sard_C2<-function(label_tb,escRecl,especie,region){
  ...
  load(Rdata_proy_Hito2)
  CBAd_marzoS0 <- t(CBAd_marzoD2SdR)
  load(Rdata_proy_Hito2S1)
  CBAd_marzoS1 <- t(CBAd_marzoD2SdR)
  load(Rdata_proy_Hito2S2)
  CBAd_marzoS2 <- t(CBAd_marzoD2SdR)
  load(Rdata_proy_Hito2S3)
  CBAd_marzoS3 <- t(CBAd_marzoD2SdR)
  load(Rdata_proy_Hito2S4)
  CBAd_marzoS4 <- t(CBAd_marzoD2SdR)
  ...
  caption1 <-"Estimación de CBA 2023 de los cuatro escenarios de supuesto de captura 2022/23
              considerando el 
              descuento del 4\\% de descarte. Criterio 2."
